...
document.addEventListener('DOMContentLoaded', async () => {
  function updateHeader(nickname) {
      ...
      navLinks.innerHTML = `
          <div class="dropdown">
              <button class="dropbtn">${nickname}</button>
...
if ((currentPage.endsWith('register.html') || currentPage.endsWith('login.html')) && localStorage.getItem('userNickname')) {
        window.location.href = 'index.html';
    }
const savedNickname = localStorage.getItem('userNickname');
if (savedNickname) {
    updateHeader(savedNickname);
    showNotification('Добро пожаловать, ' + savedNickname + '!', 'success'); 
} else {
    updateHeaderButtons();
}
    ...
    if (loginForm.querySelector('h2').textContent === 'Вход') {
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                showNotification('Вы успешно вошли в систему, ' + data.nickname + '!', 'success'); 
                loginForm.reset();
                window.location.href = "index.html";
            } catch (error) {
